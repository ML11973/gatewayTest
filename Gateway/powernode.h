...
public:
     ...
     * @param address node address
     * @param group node group
     * @param leaseDuration lease duration (multiple of NETWORK_LEASE_UNIT_MINUTES)
     */
    PowerNode(uint8_t _address, uint32_t _group, uint8_t _leaseDuration);
    /**
     * @brief Constructor from existing Node
     * @param Node source Node to transtype cast
     *
     ...
     * @param node PowerNode to write in stream
     */
...
private:
     ...
     * @param sPowerSettings power settings received from app_getPowerSettings command
