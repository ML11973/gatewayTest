...
#define BAUDRATE       20000 // must be lower than 30000 for using stop2 mode of RM1
...
static commands_t configuration_commands = {
    ...
    //{0x05, CMD_SET_PATTERN,           0x5F, 0x81, 0x04, 0xD5},
...
static void framed_rx(const uint8_t* data, uint16_t size)
{
    ...
    if (!is_initialized()) {
        if ((data[0] == 1) && (data[1] == CMD_SET_HOST_BAUDRATE)) {
            ...
            state++;
            wait_response = false;
            usleep(1000*BAUDRATE_CHANGE_DURATION);
            framed_uart_flush();
